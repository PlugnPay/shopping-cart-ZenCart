 ...
 *
 * Supports authonly or authpostauth via module configuration.
 * Capture / void / refund are handled in PlugnPay admin, not from Zen Cart.
 *
...
class plugnpay_api extends base
{
    ...
    public function __construct()
    {
        ...
        if (IS_ADMIN_FLAG === true) {
            ...
            if ($this->enabled) {
                if (!defined('MODULE_PAYMENT_PLUGNPAY_API_LOGIN')
                    ...
                    || trim((string)MODULE_PAYMENT_PLUGNPAY_API_LOGIN) === ''
        ...
        // Auth-only: keep order pending until settled outside Zen Cart
    ...
    public function after_process(): bool
    {
        ...
        $comments = 'Credit Card payment via PlugnPay Remote API. AUTH: ' . $this->auth_code .
            ' TransID/orderID: ' . $this->transaction_id;
        if (defined('MODULE_PAYMENT_PLUGNPAY_API_AUTHTYPE') && MODULE_PAYMENT_PLUGNPAY_API_AUTHTYPE === 'authonly') {
            $comments .= ' (auth-only; settle in PlugnPay Admin)';
        }
    ...
    public function install(): void
    {
        ...
        $db->Execute("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('Publisher Name', 'MODULE_PAYMENT_PLUGNPAY_API_LOGIN', '', 'PlugnPay gateway account username (publisher-name)', '6', '0', now())");
        ...
        $db->Execute("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Authorization Type', 'MODULE_PAYMENT_PLUGNPAY_API_AUTHTYPE', 'authonly', 'authonly = authorize only (settle later in PlugnPay Admin); authpostauth = authorize and settle (sale)', '6', '0', 'zen_cfg_select_option(array(\'authonly\', \'authpostauth\'), ', now())");
    ...
    {
        // Drop obsolete keys from earlier builds (test mode / refund status / admin ops)
        if (defined('MODULE_PAYMENT_PLUGNPAY_API_STATUS')) {
            global $db;
            $db->Execute(
            ...
            if (!defined('MODULE_PAYMENT_PLUGNPAY_API_AUTHTYPE')) {
                $db->Execute("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Authorization Type', 'MODULE_PAYMENT_PLUGNPAY_API_AUTHTYPE', 'authonly', 'authonly = authorize only (settle later in PlugnPay Admin); authpostauth = authorize and settle (sale)', '6', '0', 'zen_cfg_select_option(array(\'authonly\', \'authpostauth\'), ', now())");
            }
            if (!defined('MODULE_PAYMENT_PLUGNPAY_API_ORDER_STATUS_ID')) {
                $db->Execute("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) VALUES ('Set Completed Order Status', 'MODULE_PAYMENT_PLUGNPAY_API_ORDER_STATUS_ID', '2', 'Status for completed (authpostauth / sale) orders. Auth-only forces Pending (1) until settled in PlugnPay Admin.', '6', '0', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
            }
