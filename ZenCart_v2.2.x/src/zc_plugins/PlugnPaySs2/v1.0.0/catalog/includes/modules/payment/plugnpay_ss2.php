 ...
 *
 * Card data is collected on PlugnPay. After payment, PlugnPay POSTs back to
 * checkout_process. Admin capture/void/refund use the Remote API.
 *
...
class plugnpay_ss2 extends base
{
    ...
    public function __construct()
    {
        ...
        if (IS_ADMIN_FLAG === true) {
            ...
            if ($this->enabled) {
                ...
                if (!function_exists('curl_init')) {
                    $messageStack->add_session(MODULE_PAYMENT_PLUGNPAY_SS2_TEXT_ERROR_CURL_NOT_FOUND, 'error');
                }
                $this->tableCheckup();
            }
        } else {
        ...
        if (defined('MODULE_PAYMENT_PLUGNPAY_SS2_ORDER_STATUS_ID') && (int)MODULE_PAYMENT_PLUGNPAY_SS2_ORDER_STATUS_ID > 0) {
            $this->order_status = (int)MODULE_PAYMENT_PLUGNPAY_SS2_ORDER_STATUS_ID;
        }
        // Auth-only: keep order pending until capture
        if (defined('MODULE_PAYMENT_PLUGNPAY_SS2_AUTHTYPE') && MODULE_PAYMENT_PLUGNPAY_SS2_AUTHTYPE === 'authonly') {
            $this->order_status = 1;
        }
    ...
    public function process_button(): string
    {
        ...
        $description = 'Website Purchase from ' . str_replace('"', "'", STORE_NAME);
        ...
        if (isset($order->info['currency']) && $order->info['currency'] !== $this->gateway_currency) {
            ...
            $description .= ' (Converted from: ' .
                number_format((float)$order->info['total'] * (float)$order->info['currency_value'], 2) .
                ' ' . $order->info['currency'] . ')';
        }
        ...
        $submit_data = [
            ...
            'pb_port_auth' => (defined('MODULE_PAYMENT_PLUGNPAY_SS2_AUTHTYPE')
                && MODULE_PAYMENT_PLUGNPAY_SS2_AUTHTYPE === 'authpostauth')
                ? 'yes'
                : 'no',
            'pt_account_code_2' => (string)($_SESSION['customer_id'] ?? ''),
            ...
            'pb_success_url' => zen_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL', true, false),
        ...
        // Remember amount/account for basic return checks (and DB "sent" snapshot)
    ...
    public function after_process(): bool
    {
        ...
        $comments = 'Credit Card payment via PlugnPay Smart Screens v2. AUTH: ' . $this->auth_code .
    ...
    {
        $output = '';
        require __DIR__ . '/plugnpay_ss2/admin_notification.php';
        return $output;
    }
    ...
    public function install(): void
    {
        ...
        $db->Execute("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added, use_function) VALUES ('Remote Client Password', 'MODULE_PAYMENT_PLUGNPAY_SS2_PASSWORD', '', 'Remote Client Password from PlugnPay Security Administration. Required for Admin Capture / Void / Refund only. This is NOT your admin login password.', '6', '0', now(), 'zen_cfg_password_display')");
        $db->Execute("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Authorization Type', 'MODULE_PAYMENT_PLUGNPAY_SS2_AUTHTYPE', 'authonly', 'authonly = authorize only (capture later); authpostauth = authorize and settle (sale)', '6', '0', 'zen_cfg_select_option(array(\'authonly\', \'authpostauth\'), ', now())");
        ...
        $db->Execute("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) VALUES ('Set Completed Order Status', 'MODULE_PAYMENT_PLUGNPAY_SS2_ORDER_STATUS_ID', '2', 'Status for completed (sale / captured) orders. Auth-only forces Pending (1) until capture.', '6', '0', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
        $db->Execute("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) VALUES ('Set Refunded Order Status', 'MODULE_PAYMENT_PLUGNPAY_SS2_REFUNDED_ORDER_STATUS_ID', '1', 'Status for refunded orders', '6', '0', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
    ...
    public function _doRefund($oID, $amount = 0): bool
    {
        ...
        $new_order_status = (int)MODULE_PAYMENT_PLUGNPAY_SS2_REFUNDED_ORDER_STATUS_ID;
        ...
        if (!isset($_POST['refconfirm']) || $_POST['refconfirm'] !== 'on') {
            $messageStack->add_session(MODULE_PAYMENT_PLUGNPAY_SS2_TEXT_REFUND_CONFIRM_ERROR, 'error');
            $proceed = false;
        }

        $refundAmt = 0.0;
        if (isset($_POST['buttonrefund']) && $_POST['buttonrefund'] === MODULE_PAYMENT_PLUGNPAY_SS2_ENTRY_REFUND_BUTTON_TEXT) {
            $refundAmt = (float)($_POST['refamt'] ?? 0);
            if ($refundAmt <= 0) {
                $messageStack->add_session(MODULE_PAYMENT_PLUGNPAY_SS2_TEXT_INVALID_REFUND_AMOUNT, 'error');
                $proceed = false;
            }
        }
        ...
        if ($orderId === '') {
            $messageStack->add_session(MODULE_PAYMENT_PLUGNPAY_SS2_TEXT_TRANS_ID_REQUIRED_ERROR, 'error');
            $proceed = false;
        }
        ...
        if (!$api->isApproved($response)) {
            $messageStack->add_session(
                trim((string)($response['MErrMsg'] ?? MODULE_PAYMENT_PLUGNPAY_SS2_TEXT_COMM_ERROR)),
                'error'
            );
            return false;
        }
        ...
        $messageStack->add_session(sprintf(MODULE_PAYMENT_PLUGNPAY_SS2_TEXT_REFUND_INITIATED, $orderId), 'success');
    ...
    public function _doCapt($oID, $amt = 0, $currency = 'USD'): bool
    {
        ...
        $new_order_status = (int)MODULE_PAYMENT_PLUGNPAY_SS2_ORDER_STATUS_ID;
        if ($new_order_status === 0) {
            $new_order_status = (int)DEFAULT_ORDERS_STATUS_ID;
        }
        ...
        if (!isset($_POST['captconfirm']) || $_POST['captconfirm'] !== 'on') {
            $messageStack->add_session(MODULE_PAYMENT_PLUGNPAY_SS2_TEXT_CAPTURE_CONFIRM_ERROR, 'error');
            $proceed = false;
        }

        $captureAmt = (float)($_POST['captamt'] ?? 0);
        if ($captureAmt <= 0) {
            $messageStack->add_session(MODULE_PAYMENT_PLUGNPAY_SS2_TEXT_INVALID_CAPTURE_AMOUNT, 'error');
            $proceed = false;
        }
        ...
        zen_update_orders_history($oID, $comments, null, $new_order_status, 1);
        $messageStack->add_session(
            sprintf(MODULE_PAYMENT_PLUGNPAY_SS2_TEXT_CAPT_INITIATED, $formattedAmt, $orderId),
            'success'
        );
    ...
    public function _doVoid($oID, $note = ''): bool
    {
        ...
        if (!isset($_POST['voidconfirm']) || $_POST['voidconfirm'] !== 'on') {
            $messageStack->add_session(MODULE_PAYMENT_PLUGNPAY_SS2_TEXT_VOID_CONFIRM_ERROR, 'error');
            $proceed = false;
        }
        ...
        $messageStack->add_session(sprintf(MODULE_PAYMENT_PLUGNPAY_SS2_TEXT_VOID_INITIATED, $orderId), 'success');

        return true;
    }

    private function getApiClient(): PnPSs2Api
    {
        require_once __DIR__ . '/plugnpay_ss2/PnPSs2Logger.php';
        require_once __DIR__ . '/plugnpay_ss2/PnPSs2Api.php';
        ...
        return new PnPSs2Api(
            defined('MODULE_PAYMENT_PLUGNPAY_SS2_LOGIN') ? (string)MODULE_PAYMENT_PLUGNPAY_SS2_LOGIN : '',
            defined('MODULE_PAYMENT_PLUGNPAY_SS2_PASSWORD') ? (string)MODULE_PAYMENT_PLUGNPAY_SS2_PASSWORD : '',
            $logger
        );
    }
